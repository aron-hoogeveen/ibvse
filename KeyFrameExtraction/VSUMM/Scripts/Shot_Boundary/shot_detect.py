...
cmd = 'ffmpeg -i stoel.mp4 2>&1|findstr "Duration">> duration.txt'
os.system(cmd)
...
cmd = 'ffprobe -show_frames -of compact=p=0 -f lavfi "movie=stoel.mp4,select=gt(scene\,0.3)">> scenes.txt'
os.system(cmd)
...
print (' ================= ')
cmd = 'ffmpeg -i stoel.mp4 -vcodec rawvideo -acodec copy uncompressed.mp4' #pcm_s16le
os.system(cmd)
...
for line in open(seginfo,'r'):
    ...
    cmd += tbw + ' -i uncompressed.mp4 -vcodec mpeg4 -acodec copy -t ' # change codecs if necessary
    # duration
    td = te - tb
    tdh = int(td/3600)
    tdm = int((td%3600)/60)
