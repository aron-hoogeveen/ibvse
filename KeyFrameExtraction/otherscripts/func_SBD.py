...
__absolute_threshold__ = 1  # any transition must be no less than this threshold
...
class shotDetector(object):
    ...
    def run(self, video_path=None):
        ...
        return self.scores, hists, self.frames
    ...
    def pick_frame(self, obj_path=None, video_split_id=None):
        ...
        if self.video_split_id and self.obj_path:
            ...
            for i, idx in enumerate(frame_middle_idx):
                print(i, idx)
                cv2.imwrite(
                    "{0}/{1}@@{2:.1f}-{3:.1f}.jpg".format(self.obj_path, self.video_split_id, timestamp_index[i][0],
                                                          timestamp_index[i][1]), self.frames[idx - 1])
...
def merge_short_shots(shots, min_duration):
    # merge short shots
    while True:
        durations = [shot[1] - shot[0] for shot in shots]
        ...
        # can only merge right
            ...
            if durations[idx - 1] < durations[idx + 1]:
                ...
                left = False
        if left:
            shots = left_half[:-1] + [(left_half[-1][0], shot[1])] + right_half
        else:
            shots = left_half + [(shot[0], right_half[0][1])] + right_half[1:]
